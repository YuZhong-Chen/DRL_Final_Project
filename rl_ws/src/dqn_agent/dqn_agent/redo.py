...
from dqn_agent.dqn import NETWORK
...
@torch.no_grad()
def _reset_dormant_neurons(model: NETWORK, redo_masks: torch.Tensor, use_lecun_init: bool) -> NETWORK:
    ...
    for i in range(len(layers[:-1])):
        ...
        if (isinstance(layer, nn.Conv2d) and isinstance(next_layer, nn.Linear)):# and not next_layer_name == 'q':
